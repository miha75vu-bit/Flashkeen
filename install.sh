...
# Только стабильный релиз (prerelease=false, без маркера test).
stable_row="$(printf "%s\n" "$releases" | awk -F"$TAB" '
...
curl -fL -s --connect-timeout 10 --max-time 180 "$chosen_url" -o /opt/bin/flashkeen || {
  echo "Не удалось скачать по тегу, пробую releases/latest..."
  curl -fL -s --connect-timeout 10 --max-time 180 "$DEFAULT_ASSET_URL" -o /opt/bin/flashkeen
}
